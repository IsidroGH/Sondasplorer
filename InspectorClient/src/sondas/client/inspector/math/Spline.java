...
public class Spline
{
	...
	public Spline(long xx[], float ff[])
	{
...
//		Local variables:   
		double fp1, fpn, h, p;
		final double zero = 0.0f, two = 2.0f, three = 3.0f;
...
//		Evaluate cubic polynomial on [x[interval] , x[interval+1]]. 
	...
	public double integrate()
	{
		...
		if(!uniform)
		{
			...
			{
				dx = x[i+1]-x[i];
				t = (double)(t + (f[i]+(b[i]/2.0+(c[i]/3.0+dx*d[i]/4.0)*dx)*dx)*dx);
			}
			return t;
		}
//		compute uniform integral of spline fit 
		...
		t = (double)((sumd+(sumc/2.0+(sumb/3.0+dx*suma/4.0)*dx)*dx)*dx);
...
//		Assign values for data arrays x and f. 
		...
		for(int i=0; i<n; i++)
		{
			xx[i] = (float)(-2.0+0.40*i);
			ff[i] = (float)Math.pow(Math.cos(xx[i]), 4);
...
//		Calculate coefficients defining the cubic spline. 
	...
	public static void test2(){
		...
		//float xx[] = new float[]{0,1,2,3,4,5};
		long xx[] = new long[]{0,1,2,3,4,5};
		float ff[] = new float[]{0,1,2,3,4,5};

		//float xx[] = new float[]{2,3};
		//float ff[] = new float[]{2,2};
		
		Spline s = new Spline(xx, ff);
		
		double t =2.f;
		double v = s.spline_value(t);
		
		System.out.println("x="+f1.format(t)+",  f(x)="+f1.format(v));
	}
